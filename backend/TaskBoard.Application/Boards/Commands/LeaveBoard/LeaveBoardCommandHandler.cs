...
public record LeaveBoardCommand(Guid UserId, Guid BoardId) : IRequest<Result<Unit>>;

public class LeaveBoardCommandHandler : IRequestHandler<LeaveBoardCommand, Result<Unit>>
{
    ...
    public async Task<Result<Unit>> Handle(LeaveBoardCommand request, CancellationToken cancellationToken)
    {
        ...
        if (userBoard == null) return Result<Unit>.Success(Unit.Value);
